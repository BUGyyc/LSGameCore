...
namespace Lockstep.Game
{
    public partial class GameStateService : BaseGameService, IGameStateService
    {
        ...
        private void RemoveEntity<T>(T e)
            where T : BaseEntity
        {
            var t = e.GetType();
            if (_type2Entities.TryGetValue(t, out var lstObj))
        ...
        public T CreateEntity<T>(EntityType type, int prefabId, LVector3 position)
            where T : BaseEntity, new()
        {
            var baseEntity = new T();
            _gameConfigService.GetEntityConfig(type, prefabId)?.CopyTo(baseEntity);
            baseEntity.EntityId = _idService.GenId();
            baseEntity.PrefabId = prefabId;
            baseEntity.entityType = (uint)type;
            ...
            _debugService.Trace(
                $"CreateEntity {prefabId} pos {prefabId} entityId:{baseEntity.EntityId}"
            );
        ...
        void BackUpEntities<T>(T[] lst, Serializer writer)
            where T : BaseEntity, IBackup, new()
        {
        ...
        List<T> RecoverEntities<T>(List<T> lst, Deserializer reader)
            where T : BaseEntity, IBackup, new()
        {
