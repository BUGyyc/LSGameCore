...
namespace Lockstep.Game
{
    ...
    {
        public DumpHelper(IServiceContainer serviceContainer, World world, HashHelper hashHelper)
            : base(serviceContainer, world)
        {
            this._hashHelper = hashHelper;
        }

        public Dictionary<int, StringBuilder> _tick2RawFrameData =
            new Dictionary<int, StringBuilder>();
        public Dictionary<int, StringBuilder> _tick2OverrideFrameData =
            new Dictionary<int, StringBuilder>();
        ...
        private string dumpPath =>
            Path.Combine(
                ...
                _serviceContainer.GetService<IGameConfigService>().DumpStrPath
            );
#endif
        ...
        public void DumpToFile(bool withCurFrame = false)
        {
            ...
            if (withCurFrame)
            {
                ...
                File.WriteAllText(
                    dumpPath + "/raw_single.txt",
                    _tick2RawFrameData[Tick].ToString()
                );
            }
        ...
        public void DumpAll()
        {
            ...
            File.WriteAllText(
                dumpAllPath
                    + $"/All_{_serviceContainer.GetService<IConstStateService>().LocalActorId}.txt",
                sbRaw.ToString()
            );
        }
