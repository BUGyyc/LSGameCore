...
namespace YooAsset
{
	...
	internal abstract class ProviderBase
	{
		...
		/// </summary>
		/// <value></value>
		protected BundleLoaderBase OwnerBundleLoader { private set; get; }
		...
		public ProviderBase(AssetSystemImpl impl, string providerGUID, AssetInfo assetInfo)
		{
			...
			if (impl != null)
			{
				OwnerBundleLoader = impl.CreateOwnerAssetBundleLoader(assetInfo);
				OwnerBundleLoader.Reference();
				OwnerBundleLoader.AddProvider(this);
		...
		public virtual void Destroy()
		{
			...
			// 释放资源包加载器
			if (OwnerBundleLoader != null)
			{
				OwnerBundleLoader.Release();
				OwnerBundleLoader = null;
			}
		...
		internal DownloadReport GetDownloadReport()
		{
			...
			result.TotalSize = (ulong)OwnerBundleLoader.MainBundleInfo.Bundle.FileSize;
			result.DownloadedBytes = OwnerBundleLoader.DownloadedBytes;
		...
		internal void GetBundleDebugInfos(List<DebugBundleInfo> output)
		{
			...
			bundleInfo.BundleName = OwnerBundleLoader.MainBundleInfo.Bundle.BundleName;
			bundleInfo.RefCount = OwnerBundleLoader.RefCount;
			bundleInfo.Status = OwnerBundleLoader.Status.ToString();
