...
namespace YooAsset.Editor
{
	...
	public class TaskCreatePackage : IBuildTask
	{
		...
		private void CopyPackageFiles(BuildParametersContext buildParametersContext, BuildMapContext buildMapContext)
		{
			...
			LogMaster.Log($"开始拷贝补丁文件(pipelineOutputDirectory):{pipelineOutputDirectory}   到补丁包目录(packageOutputDirectory)：{packageOutputDirectory}");
			...
			//! 说明 Build 之后，文件其实刚开始生成在 OutputCache 下，然后从 OutputCache 拷贝进对应 XXXX-时间序号的文件夹下
			// 拷贝所有补丁文件
			int progressValue = 0;
			...
			foreach (var bundleInfo in buildMapContext.Collection)
			{
				...
				LogMaster.L($"[CopyFile] from:{bundleInfo.BundleInfo.BuildOutputFilePath}  to:{bundleInfo.BundleInfo.PackageOutputFilePath} ");
